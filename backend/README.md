...
## Кэш ответов (`/api/search`)

Полное кэширование ответов: повторный ИДЕНТИЧНЫЙ вопрос отдаётся из sqlite мгновенно,
без планировщика/ретривала/синтеза (LLM не дёргается). Модуль `app/answer_cache.py`,
файл `backend/answer_cache.sqlite`. Не путать с `plan_cache.sqlite` (кэш интента) и
`search_cache` в `audit.sqlite` (последние N ответов по `search_id` для экспорта).

**Ключ** = `sha256(` нормализованный запрос `⨝` канонизированные фильтры `⨝` роль `⨝`
`data_version` `)`:
- запрос: `lower`, схлопнутые пробелы, `ё→е`;
- фильтры: пустые/`all` выброшены, JSON с сортированными ключами;
- роль: `researcher` по умолчанию (RBAC-партиция — external_partner не заберёт кэш
  исследователя);
- **`data_version`** — критично для инвалидации. Собирается из живых сигналов роста
  данных: `ES doc count` + `ES chunk count` + `Neo4j node count` + `Neo4j edge count`
  + `mtime` активной матрицы эмбеддингов (`graph/embeddings/<space>/chunk_embeddings.npy`).
  Любой рост (экстракция, синк, чекпоинт прекомпьюта) меняет версию → старый ключ больше
  не совпадает → пересчёт. Устаревший кэш опаснее его отсутствия. `data_version`
  кэшируется в памяти на 15 с, чтобы не гонять ES/Neo4j на каждый запрос.

**TTL**: обычный грунтованный ответ — 24 ч; честный «не найдено» (0 цитат, `synth=empty`)
— 15 мин (тема может появиться при росте корпуса); ответ с fallback-шаблоном
(`synth=template`, LLM недоступен) и ошибки — **НЕ кэшируются** (деградированный ответ не
должен выдавать себя за настоящий).

**Хук** в `search.py`: перед пайплайном — `lookup` (при хите — мгновенная отдача,
`cached=true`, `took_ms` = реальная латентность лукапа, `hit_count++`); после успешного
синтеза — `store` (с типом синтеза для выбора TTL). Параметр запроса `skip_cache=true`
(`POST /api/search?skip_cache=true`) обходит и чтение, и запись — для демо «вживую» и
честного замера харнессом.

**Стат/очистка**: `GET /api/stats` → блок `cache {entries, hits, hit_rate, size_mb}`;
`DELETE /api/cache` — очистить (RBAC отменён). Прогрев перед демо/защитой:
`../.venv-c/bin/python warm_cache.py` (4 golden + топ `qa/eval_set.yaml`, ~15 запросов,
второй проход подтверждает `cached=true` и <300 мс).

**Замер (живой, реальный корпус)**: холодный запрос `≈8.5 с` → повторный **`took_ms=1`,
wall `≈7 мс`** (тот же набор цитат). `skip_cache=true` — снова `≈8.4 с`, `cached=false`.

**Харнесс**: измеряет ЧЕСТНО без кэша — `qa/harness.py` шлёт `skip_cache=true` по
умолчанию (env `QA_USE_CACHE=1` включает кэш-путь). На метрики качества кэш не влияет.

**Тесты**: `tests/test_answer_cache.py` (10) — хит/мисс, инкремент `hit_count`,
инвалидация по `data_version` (в т.ч. симуляция роста данных), нормализация запроса,
партиция по фильтрам/роли, TTL (короткий для empty, живучесть 24 ч), пропуск
fallback-шаблона, `clear`/`stats`. Без Neo4j/ES (data_version передаётся явно).
